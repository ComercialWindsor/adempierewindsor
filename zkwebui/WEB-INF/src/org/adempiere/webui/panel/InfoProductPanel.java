...
public class InfoProductPanel extends InfoPanel implements EventListener
{
	...
	public String getSQLWhere()
	{
		...
		if (!(name.equals("") || name.equals("%")))
			//where.append(" AND UPPER(p.Name) LIKE ?"); faaguilar OFB comment original code
			if(!newView())
				where.append(" AND (UPPER(p.Name) LIKE ? or UPPER(p.Value) LIKE '%"+name+"%' )"); //faaguilar OFB
			else
				where.append(" AND (UPPER(p.Name) LIKE ? OR UPPER(p.Name2) LIKE '"+name+"' or UPPER(p.Value) LIKE '%"+name+"%' )"); //faaguilar OFB
